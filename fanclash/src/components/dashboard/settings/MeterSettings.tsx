...
export default function MeterSettings({ config, onChange }: {
  ...
  return (
    <>
      <div>
        <label className="block text-sm text-gray-400 mb-1">표현 스타일</label>
        <select value={(config.skin as string) || 'gauge'}
          onChange={e => onChange({ ...config, skin: e.target.value })}
          className="w-full bg-gray-800 border border-gray-600 rounded-lg px-3 py-2 text-sm">
          <option value="gauge">온도 게이지</option>
          <option value="weather">날씨 효과</option>
        </select>
        <p className="text-xs text-gray-600 mt-1">게이지 바 또는 날씨 파티클 효과 중 선택</p>
      </div>
      <div>
        ...
        <p className="text-xs text-gray-600 mt-1">이 시간 동안의 후원 총액으로 분위기를 측정합니다</p>
      </div>
      {(config.skin as string) !== 'weather' && (
        <>
          <div>
